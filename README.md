...
The number 7238234792 is a unique integer generated by the time and date in milliseconds, to make the marker unique.  Mathematical Expressions, Citation and References are extracted and replaced by an `encode()`-call of `wtf_tokenizer`. The tokenizer is defined in `/src/index.js` and requires the submodules.
For further processing e.g. with the [`wtf_wikipedia` library](https://github.com/spencermountain/wtf_wikipedia) the tokens/markers are regarded as ordinary words in text.
...
<kbd>npm install wtf_tokenizer</kbd>
